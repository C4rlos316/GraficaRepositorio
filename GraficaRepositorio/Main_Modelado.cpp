...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//TABLA
	    model = glm::mat4(1.0f);
		// scalado sobre el eje y mas chico para parecer una tablita 
		model = glm::scale(model, glm::vec3(3.0f, 0.1f, 2.0f));
		//Traslacion sobre el Y
	    model = glm::translate(model, glm::vec3(0.0f, 5.8f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);


		//PATITA DERECHA ARRIBA
		model = glm::mat4(1);
		model = glm::scale(model, glm::vec3(0.1f, 0.9f, 0.1f));
		model = glm::translate(model, glm::vec3(10.6f, 0.2f, 8.8f));

		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);

		//PATITA DERECHA ABAJO
