...
int main( )
{
    ...
    // Load models
    //proyeccion para el model y cargar el modelo en esta parte
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //perrito 2 trasladado sobre el eje -X 
        model = glm::translate(model, glm::vec3(2.25f, 0.0f, 0.0f));
        model = glm::scale(model, glm::vec3(2.25f, 2.35f, 3.0f));
        glUniformMatrix4fv(glGetUniformLocation(shader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));
        dog.Draw(shader);

        //gato trasladado sobre el eje X 
        model = glm::translate(model, glm::vec3(-2.25f, 0.0f, 0.0f));
